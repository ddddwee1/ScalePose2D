...
class ModelWithLoss(M.Model):
	...
	def forward(self, batch):
		...
		for i in range(len(config.inp_scales)):
			...
			model_outputs = self.model(img_resized)
			out_hmap = model_outputs[i]
			hm = self.HM(out_hmap[:,0:1], hmap, mask.unsqueeze(1))
			offls = self.Off(out_hmap[:,1:], offset_gt, offset_weight)

			fused = model_outputs[-1]
			of_gt = batch[i*4+1]
			of_w = batch[i*4+2]
			hm_gt = batch[i*4+3]
			hm_msk = batch[i*4+4]
			...
			all_offls.append(offls)

			all_maps_fused.append(fused)
			all_losses_fused.append(hm_fused)
			all_offls_fused.append(offls_fused)

		return all_losses, all_offls, all_hmaps, all_losses_fused, all_offls_fused, all_maps_fused
