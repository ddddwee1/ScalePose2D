...
def main_worker(gpu, ngpus_per_node):
	...
	for e in range(config.max_epoch):
		...
		for i, batch in enumerate(loader):
			...
			optim.zero_grad()
			hmap_loss_fused, offs_loss_fused, outs_fused = model(batch)
			...
			loss_total = hmap_loss_f + offs_loss_f 
			loss_total.backward()
			optim.step()
			...
			if i%100==0 and gpu==0:
				...
				visutil.vis_batch(img, hmap, './outputs/%d_outgt0.jpg'%i)
				visutil.vis_batch(img, batch[3], './outputs/%d_outgt1.jpg'%i)
				visutil.vis_batch(img, batch[7], './outputs/%d_outgt2.jpg'%i)

				visutil.vis_offset(img, outs_fused[0][:,1:] * config.offset_scale, pts * config.scales[0], './outputs/%d_off_f0.jpg'%i)
				visutil.vis_offset(img, outs_fused[1][:,1:] * config.offset_scale, pts * config.scales[1], './outputs/%d_off_f1.jpg'%i)
				visutil.vis_offset(img, outs_fused[2][:,1:] * config.offset_scale, pts * config.scales[2], './outputs/%d_off_f2.jpg'%i)
				visutil.vis_offset(img, offset_map * config.offset_scale, pts, './outputs/%d_offgt2.jpg'%i)
				visutil.vis_offset(img, batch[1] * config.offset_scale, pts * config.scales[0], './outputs/%d_offgt0.jpg'%i)
				visutil.vis_offset(img, batch[5] * config.offset_scale, pts * config.scales[1], './outputs/%d_offgt1.jpg'%i)
