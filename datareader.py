...
class COCO_kpts(Dataset):
    def __init__(self):
        ...
        self.aff = RandomAffineTransform(config.inp_size, config.out_size, config.rotation,
                                         config.min_scale, config.max_scale, 'short', config.max_translate, True)
    ...
    def _get_pts(self, pts_list, reorder):
        ...
        for item in pts_list:
			...
			# try dynamic sigma but failed to converge - Feb 2021
			# Now I think it's because the sigma value is not properly set, but I dont want to try again - June 2021
            # sigma = config.base_sigma * area / config.out_size
            # box size cannot reflect the person's scale, so dynamic sigma using box size could be inappropriate
    ...
    def __getitem__(self, index):
        ...
        for i in range(pts.shape[0]):
            hmap = rc.render_heatmap(
                pts[i, :, :3].copy(), config.out_size, config.base_sigma)
            hmaps.append(hmap)
        hmaps = np.float32(hmaps)
        hmap = np.amax(hmaps, axis=0)
        pts_res = np.zeros(
            [config.max_inst, config.num_pts, 3], dtype=np.float32)
